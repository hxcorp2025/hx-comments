...
> As migrations `evo_*` (instâncias Evolution, warmup, leitura de grupos) vivem no MESMO projeto
> Supabase, mas **quem usa é o Gestor de Grupos HX** (`send.hx-corp.com`, repo `gghx-disparado`).
> O runbook delas está em `backend/README.md` DAQUELE repo. Nada nesta Central chama `evo_*`.
